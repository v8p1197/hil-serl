...
class EnvConfig(DefaultEnvConfig):
    ...
    REALSENSE_CAMERAS = {
        "wrist_1": {
            "serial_number": "218622276769",
            "dim": (1280, 720),
            "exposure": 40000,
        },
    }
    IMAGE_CROP = {
        "wrist_1": lambda img: img[300:800, 400:1100],
    }
    TARGET_POSE = np.array([0.44,-0.068,0.067, np.pi, 0, 0])
    GRASP_POSE = np.array([0.44,-0.068,0.067, np.pi, 0, 0])
    RESET_POSE = TARGET_POSE + np.array([0, 0, 0.1, 0, 0.0, 0])
    ABS_POSE_LIMIT_LOW = TARGET_POSE - np.array([0.03, 0.02, 0.01, 0.01, 0.1, 0.4])
    ABS_POSE_LIMIT_HIGH = TARGET_POSE + np.array([0.03, 0.02, 0.15, 0.01, 0.1, 0.4])
    RANDOM_RESET = True
    ...
    def get_environment(self, fake_env=False, save_video=False, classifier=False):
        ...
        if classifier:
            ...
            def reward_func(obs):
                ...
                condition = sigmoid(classifier(obs)) > 0.85 and obs['state'][0, 6] > 0.04

                return jnp.asarray(condition, dtype=jnp.int32)
